...
const router = createBrowserRouter(
    Object.values(ROUTE_CONFIG).map(({ path, element, isProtected, isGuestOnly, roles = [] }) => ({
        path,
        element: isProtected ? <ProtectedRoute roles={roles}>{element}</ProtectedRoute> : isGuestOnly ? <GuestRoute>{element}</GuestRoute> : element,
    })),
);
...
export const PATHS = Object.fromEntries(Object.entries(ROUTE_CONFIG).map(([key, value]) => [key, value.path]));
